...
export default function AddLoan() {
  ...
  useEffect(() => {
    api.workers.list({ page:1, pageSize:100 })
  ...
  const onSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await api.loans.create({ workerId: form.workerId, amount: Number(form.amount), loanDate: form.loanDate, notes: form.reason });
      setSuccess("Loan saved");
  ...
  return (
    ...
    {error && <div className="mb-4 rounded-md border border-rose-300 bg-rose-50 px-3 py-2 text-sm text-rose-700">{error}</div>}
        <form className="grid grid-cols-1 gap-4 sm:grid-cols-2" onSubmit={onSubmit}>
          ...
          <div>
            <DatePicker value={form.loanDate} onChange={(e)=> setForm(f=> ({...f, loanDate: e.target.value}))} />
